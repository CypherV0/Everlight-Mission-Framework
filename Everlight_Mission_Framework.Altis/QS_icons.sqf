...
_QS_ST_colorInjured = [0.7,0.6,0,0.5];						// ARRAY (NUMBER). RGBA color code. Color of units with > 10% damage, in map group interactive interface. Default [0.7,0.6,0,0.5];
...
_QS_ST_groupIconScale = 0.70;										// NUMBER. Group Icon Scale. Default = 0.75
...
_QS_fnc_iconColor = {
	...
	if (_useTeamColor) then {
		if (isNull (objectParent _u)) then {
			...
			if (_u isEqualTo player) then {_c = [1,1,1,1];};		// Sets the player icon color to White.
			_c set [3,_a];
			if (_ms > 0.80) then {
				if (_ds isEqualTo 1) then {
					_c set [3,0];
